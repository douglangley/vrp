...
main.py  (entry; applies the chirp import-path fix, then runs the app)
  └─ vrp/app.py : VRPApp / MainWindow (wx.Frame)
       ├─ native wx menu bar (File / Radio / Channels / Help)
...
- **Three command surfaces, kept in sync** (see CLAUDE.md "Command Surfaces"):
  ...
  so the bridged in-page shortcuts are the reliable key path; the menu still
  works via Alt → arrows → Enter and restores webview focus after.
...
Nuitka single-exe with CHIRP bundled (`build.py`). See README "Packaging with
Nuitka". Validated in Phase 9.
