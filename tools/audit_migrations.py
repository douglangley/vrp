#!/usr/bin/env python
r"""Smoke-test VRP's migration wrapper against every pinned CHIRP image.

This is intentionally an opt-in audit rather than part of the quick unit-test
suite: the pinned CHIRP tree currently contains hundreds of driver fixtures and
the sweep takes roughly a minute. An ``incompatible`` result is expected for
radios whose band or feature set cannot represent the sample channel; a
``failed`` result means VRP or a driver raised outside CHIRP's normal
compatibility contract and makes the command fail.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="directory containing pinned CHIRP .img fixtures",
    )
    parser.add_argument(
        "--source-channel",
        type=int,
        default=25,
        help="channel from CHIRP's Generic_CSV.csv to migrate (default: 25)",
    )
    ...
    batch = migration.batch_from_radio(source, numbers=[args.source_channel])
    if not batch.entries:
        print(f"Generic_CSV.csv channel {args.source_channel} is empty or unreadable")
        return 2
    ...
    for image_path in images:
        try:
            ...
            for index, target in enumerate(targets):
                low, _high = target.get_features().memory_bounds
                report = migration.apply_batch(target, batch, low)
                status = report.items[-1].status if report.items else "no result"
                counts[status] += 1
                if report.failed or not report.items:
                    failures.append(
                        f"{image_path.name} subdevice {index}:\n"
                        f"{report.details_text()}"
                    )
        except Exception as exc:  # noqa: BLE001 - audit must continue all images
    ...
    print(f"Audited {total} radio targets from {len(images)} image files.")
