...
REM  via uv, then launches the app. Subsequent runs just launch (fast). Any
REM  arguments (e.g. --debug) are passed through to main.py.
REM
REM  Prerequisites (one-time):
REM    - uv     ->  winget install --id=astral-sh.uv
REM    - git    ->  https://git-scm.com/download/win  (Git for Windows)
REM  The UI is built entirely from native wx controls — no webview or browser,
REM  so no WebView2 runtime is required.
REM ===========================================================================
setlocal
cd /d "%~dp0"
